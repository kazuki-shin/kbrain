...
export async function importFixtures() {
  ...
  for (const filePath of files) {
    ...
    const result = await importFromContent(e, parsed.slug, content, { noEmbed: true, noEnrich: true });
    results.push(result);
  }
...
export async function importFixture(relativePath: string) {
  ...
  return importFromContent(e, parsed.slug, content, { noEmbed: true, noEnrich: true });
}
